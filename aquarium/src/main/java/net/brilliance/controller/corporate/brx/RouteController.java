...
@Slf4j
@Controller
@RequestMapping(ControllerConstants.URI_BRX_ROUTE)
public class RouteController extends BaseController { 
   ...
   */
	...
	public String createForm(Model model) {
		...
		.builder()
		.build();
		model.addAttribute(net.brilliance.common.CommonConstants.FETCHED_OBJECT, newRoute);
	...
	*/
	...
	public String create(@Valid Route uiBizObject, BindingResult bindingResult,
	...
	protected List<SelectItem> suggestItems(String keyword) {
		...
		Page<Route> fetchedObjects = this.businessService.searchObjects(keyword, null);
		for (Route bizObject :fetchedObjects.getContent()) {
	...
	protected String performSearch(SearchParameter params) {
		...
		Page<Route> pageContentData = businessService.search(parameters);
