...
@Slf4j
@Controller
@RequestMapping(ControllerConstants.URI_BUSINESS_SUB_PACKAGE)
public class BusinessSubPackageController extends BaseController { 
	private String PAGE_CONTEXT_PREFIX = ControllerConstants.CONTEXT_WEB_PAGES + "corporate/brx/businessSubPackage";
   ...
   */
	...
	public String createForm(Model model) {
		BusinessSubPackage newBusinessSubPackage = BusinessSubPackage
		.builder()
		.build();
		model.addAttribute(net.brilliance.common.CommonConstants.FETCHED_OBJECT, newBusinessSubPackage);
	...
	*/
	...
	public String create(@Valid BusinessSubPackage uiBizObject, BindingResult bindingResult,
	...
	protected List<SelectItem> suggestItems(String keyword) {
		...
		Page<BusinessSubPackage> fetchedObjects = this.businessService.searchObjects(keyword, null);
		for (BusinessSubPackage bizObject :fetchedObjects.getContent()) {
	...
	protected String performSearch(SearchParameter params) {
		...
		Page<BusinessSubPackage> pageContentData = businessService.search(parameters);
