...
@Slf4j
@Controller
@RequestMapping(ControllerConstants.URI_BUSINESS_PACKAGE)
public class BusinessPackageController extends BaseController { 
   ...
   */
	...
	public String createForm(Model model) {
		BusinessPackage newBusinessPackage = BusinessPackage
		.builder()
		.build();
		model.addAttribute(net.brilliance.common.CommonConstants.FETCHED_OBJECT, newBusinessPackage);
	...
	*/
	...
	public String create(@Valid BusinessPackage uiBizObject, BindingResult bindingResult,
	...
	protected List<SelectItem> suggestItems(String keyword) {
		...
		Page<BusinessPackage> fetchedObjects = this.businessService.searchObjects(keyword, null);
		for (BusinessPackage bizObject :fetchedObjects.getContent()) {
	...
	protected String performSearch(SearchParameter params) {
		...
		Page<BusinessPackage> pageContentData = businessService.search(parameters);
