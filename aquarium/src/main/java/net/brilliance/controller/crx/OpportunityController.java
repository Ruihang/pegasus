...
@Slf4j
@Controller
@RequestMapping(ControllerConstants.URI_OPPORTUNITY)
public class OpportunityController extends BaseController { 
	private String PAGE_CONTEXT_PREFIX = ControllerConstants.CONTEXT_WEB_PAGES + "crx/opportunity";
  ...
  private TaskExecutor taskExecutor;
	...
	@Inject
	private OpportunityService businessManager;
   ...
   */
	...
	*/
	...
	public String create(@Valid Opportunity uiBizObject, BindingResult bindingResult,
	...
	protected List<SelectItem> suggestItems(String keyword) {
		...
		Page<Opportunity> fetchedObjects = this.businessManager.searchObjects(keyword, null);
		for (Opportunity bizObject :fetchedObjects.getContent()) {
	...
	protected String performSearch(SearchParameter params) {
		...
		Page<Opportunity> pageContentData = businessManager.search(parameters);
