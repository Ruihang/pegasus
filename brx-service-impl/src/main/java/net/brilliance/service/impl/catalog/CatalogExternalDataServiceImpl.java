...
public class CatalogExternalDataServiceImpl implements CatalogExternalDataService {
	 ...
	 * 
	 * @param inputStream
	 *          The source input stream of external data
	 * @param params
	 *          The additional parameters
	 * @return The execution status message
	 * @throws CatalogExternalDataException
	 *           If any error causes during the execution.
	 */
	@Override
	...
	private void parseCatalogues(List<List<String>> bucketDataElements, int startedIndex)
			throws CatalogExternalDataException {
		...
		try {
			...
			for (int elIdx = startedIndex; elIdx < bucketSize; elIdx++) {
				...
				if (CommonUtility.isNotEmpty(elements.get(0)) && CommonUtility.isEmpty(elements.get(1))
						&& CommonUtility.isEmpty(elements.get(2))) {
					if (Pattern.compile(CatalogCodeRegExp.PART.getRegExp()).matcher(elements.get(0)).find()) {// Super catalog (Part)
						...
						try {
							...
							currentPartCatalog = this.catalogRepository.getOne(catalog.getId());
							elIdx = (Integer) buckets[1] - 1;
						} catch (Exception e) {
							log.error("Error at part: " + elements, e);
						}
						continue;
						...
						try {
							...
							currentSectionCatalog = this.catalogRepository.getOne(catalog.getId());
							elIdx = (Integer) buckets[1] - 1;
						} catch (Exception e) {
							log.error("Error at section: " + elements, e);
						}
						continue;
					}
				} else if (Pattern.compile(CatalogCodeRegExp.CATALOG.getRegExp()).matcher(elements.get(0)).find()
						&& (elements.get(0).length() >= 5 && elements.get(0).length() <= 7)) {// Catalog
					...
					if (!catalogOptional.isPresent()) {
						...
						try {
							...
							log.error("Error at catalogue: " + elements, e);
						}

						if (elements.get(0).length() < 7) {
							parentCatalog = this.catalogRepository.getOne(catalog.getId());
						}
					}
					continue;
				} else if (Pattern.compile(CatalogCodeRegExp.CATALOG.getRegExp()).matcher(elements.get(0)).find()
						&& 10 == elements.get(0).length()) {// Product|Inventory Item
						...
						if (unitMap.containsKey(elements.get(2))) {
							...
							if (!unitOptional.isPresent()) {
								...
								try {
									...
									log.error("Error at measure unit: " + elements, e);
								}
							} else {
					...
					try {
						...
						log.error("Error at product/inventory: " + elements, e);
					}
					...
					try {
						...
						log.error("Error at product-catalogue: " + elements, e);
					}
	...
	private Object[] buildFullCatalog(int idx, List<List<String>> bucketDataElements, String terminator)
			throws EcosysException {
		int elIdx = idx;
